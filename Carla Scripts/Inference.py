...
def detect_sign(img, model_path:str):
    assert not isinstance(img,type(None)), 'origin_img not found'
    img, ratio = preprocess(img, [640, 640])
    assert not isinstance(img,type(None)), 'origin_img  aa not found'
    ...
    if dets is not None:
        ...
        # else:
        #     print(-1.0)
        return float(CLASSES[cls_ind]), score
    return None